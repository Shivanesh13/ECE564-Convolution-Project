...
module tb;
  ...
  localparam ADJUST_ADDRESS_WIDTH = ADDRESS_WIDTH - (TB_DQW>>ADDRESS_OFFSET_WIDTH);
  ...
  initial begin
    $display("[TB] cfg: dqwidth=%0d, burstlen=%0d, rdlat=%0d", TB_DQW, TB_BURST, TB_RDLAT);
  end
  ...
  //--------------------------------------------------------------------------
  logic [1:0]  		      CMD[2];
  logic [ADDRESS_WIDTH-1:0]   addr[2];
  wire  [TB_DQW-1:0] 	      DQ[2];           // actual bidirectional bus to mem_ctrl
  logic                       start;
  wire                        ready;

  logic [TB_DQW-1:0] 	      din[2];   // TB drive data (we mask to TB_DQW)
  logic [TB_DQW-1:0] 	      dout[2];    // TB sampled bus
  ...
  for(genvar i=0;i<2;i++) begin : mem_block
    ...
    tri_state_driver dut_dram[TB_DQW-1:0] (
      .din 	(din[i]),
      .oe	(oe[i]),
      .pad    	(DQ[i]),
      .dout 	(dout[i])
  ...
  int timeout_limit = 5000000;
  initial begin
    while(timeout_limit--) begin
      @(posedge clk);
      if(start)
